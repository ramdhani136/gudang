<template>
  <div class="container">
    ...
    <div class="row mt-2">
      <button @click="aktiftombol = !aktiftombol" v-if="status === 'Confirmed'" class="btn-orange btn ml-3">{{ namatombol }} {{ aktiftombol ? edit() : close() }}</button>
      <button @click="requestulang()" v-if="status === 'Confirmed' && namatombol === 'Close'" class="btn-primary btn ml-2">Request Ulang</button>
      <button @click="hapuspr()" v-if="status === 'Confirmed' || status === 'Purch'" class="btn-danger btn ml-2">Batalkan PR</button>
      <button @click="createso()" v-if="status === 'Confirmed' && namatombol === 'Edit'" class="btn-success btn ml-2">Kirim PR</button>
      <button @click="kembali()" v-if="status === 'So'" class="btn-primary btn ml-3">Kembali</button>
    </div>
...
export default {
  ...
  methods: {
    getRso() {
      axios.get("/api/rso/data/pr/" + this.$route.params.nomor).then((res) => {
        this.rso = res.data.data;
        this.status = this.rso[0].status;
        this.rsoketerangan = this.rso[0].keterangan;
        axios.get("/api/listrso/" + this.$route.params.nomor).then((res) => {
        ...
        axios.get("/api/listrso/data/pr/" + this.$route.params.nomor).then((res) => {
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios
              .put("/api/rso/" + this.$route.params.nomor, {
                status: "Purch",
              ...
              .then((res) => {
                ...
                swalWithBootstrapButtons.fire("Terkirim!", "Silahkan tunggu konfirmasi purchasing.", "success");
            ...
            swalWithBootstrapButtons.fire("Cancelled", "Batal melakukan permintaan ini :)", "error");
          }
        });
    },
    createso() {
            ...
            if (result.isConfirmed) {
              axios
                ...
                .then((res) => {
                  ...
                  axios.post("/api/so", this.cso).then((res) => {
                    for (let i = 0; i < this.listsoacc.length; i++) {
                      this.uplist = {
                        ...
                        idx: this.listsoacc[i].idx,
                      };
                      axios.post("/api/listso", this.uplist);
                    }
                    swalWithBootstrapButtons.fire("Terkirim!", "Berhasil mengirim permintaan.", "success");
                    axios.post("/api/history", {
                      nomor_dok: this.$route.params.nomor,
                      id_user: this.ambiluser.id,
    ...
    hapuspr() {
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios.delete("/api/rso/" + this.$route.params.nomor).then((res) => {
            ...
            swalWithBootstrapButtons.fire("Cancelled", "Batal menghapus PR ini :)", "error");
          }
        });
    },
    DateTime() {
