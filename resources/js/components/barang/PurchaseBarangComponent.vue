        ...
        <table id="thead" class="table table-striped table-bordered" style="width:100%">
            ...
            <tbody>
                <tr v-for="(br , index) in filteredMembers" :key="br.kode">
                    ...
                    <td>{{br.kode}}</td>
                    <td>{{br.nama}}</td>
                    <td style="text-align:center">{{br.kategori}}</td>
                    <td style="text-align:center">{{br.satuan}}</td>
                    <td style="text-align:center">
                        <button @click="updateBarang(br)" class="btn btn-primary">Lihat</button>
                        <button v-if="status==='N'" @click="deleteBarang(br)" class="btn btn-danger">Batalkan</button>
                    </td>
                </tr>
            </tbody>
        </table>
            ...
            <div id="modal-width" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select v-model="form.id_kategori" class="form-control" :disabled="disabled == 1">
                            <option :value="kat.id" v-for="(kat,index) in kategorib" :key="index">
                                {{kat.nama}}
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select v-model="form.satuan" name="satuan" id="" class="form-control" :disabled="disabled == 1">
                            <option v-for="(sat,index) in satuan" :key="index" :value="sat.satuan">{{sat.satuan}}</option>
                        </select>
                        <div class="form-group">
...
export default {
    ...
    methods: {
        getBarang() {
            axios.get("/api/barang/data/all/")
                .then(res => {
                    ...
                    axios.get("/api/kategoribarang")
                        .then(res => {
                            this.kategorib = res.data.data;
        ...
        updateBarang(barang) {
            ...
            this.form.id_kategori = barang.id_kategori
            this.edit = true;
            $("#modal-form").modal("show");
        },
        deleteBarang(barang) {
