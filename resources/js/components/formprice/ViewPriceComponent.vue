<template>
  <div class="container">
    ...
    <div class="row mt-2">
      <button v-if="status !== 'Confirm' && ambiluser.sales === 1" class="btn btn-danger ml-3" @click="batalkan()">Batalkan</button>
      <button v-if="status !== 'Draft' && status !== 'Confirm' && ambiluser.sales === 1" class="btn btn-orange ml-1" @click="reqedit()">Request Edit</button>
      <button v-if="status === 'Confirm' && ambiluser.sales === 1" class="btn btn-none ml-4" @click="print()">Print</button>

      <button v-if="status === 'Draft' && ambiluser.sales === 1" class="btn btn-primary ml-1" @click="createDraft()">Simpan Draft</button>
      <button v-if="status === 'Draft' && ambiluser.sales === 1" @click="submit()" class="btn-success btn ml-1">Kirim Permintaan</button>
      <button v-if="status === 'Kordinator' && ambiluser.kordisales === 1" @click="konfirmharga()" class="btn-success btn ml-3">Kirim Konfirmasi</button>
      <button v-if="buttonkonfirm" @click="spvkonfirmasi()" class="btn-success btn ml-3">Konfirmasi</button>
    </div>
...
export default {
  ...
  methods: {
    getBarang() {
      axios.get("/api/formprice/" + this.$route.params.nomor).then((res) => {
        ...
        axios.get("/api/view/data/" + this.$route.params.nomor).then((res) => {
          this.listprice = res.data.data;
          for (let t = 0; t < this.listprice.length; t++) {
            axios.get("/api/view/price/" + this.ket2.kode + "/" + this.listprice[t].kode_barang).then((res) => {
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            if (this.listpr.length > 0) {
              ...
              if (this.aplus === this.aband) {
                if (this.upload.nomor_price === this.$route.params.nomor.slice(0, 15)) {
                  ...
                  this.upload.tgl_sent = this.DateTime();
                  axios
                    .put("/api/formprice/" + this.$route.params.nomor, this.upload)
                    .then((res) => {
                      axios
                        .get("/api/view/data/" + this.$route.params.nomor)
                        .then((res) => {
                        ...
                        .then((res) => {
                          ...
                          axios
                            ...
                            .then((res) => {
                              this.load = false;
                              swalWithBootstrapButtons.fire("Sukses!", "Berhasil mengirim permintaan harga.", "success");
                              this.$router.push({
                                name: "formprice",
                              });
                            });
                  ...
                  axios
                    ...
                    .then((res) => {
                      axios
                        .get("/api/view/data/" + this.upload.nomor_price)
                        .then((res) => {
                          this.listcust = res.data.data;
                          for (let y = 0; y < this.listcust.length; y++) {
                            axios.delete("/api/custprice/" + this.listcust[y].id);
                        ...
                        .then((res) => {
                          ...
                          axios
                            .get("/api/history/" + this.$route.params.nomor)
                            .then((res) => {
            ...
            this.load = false;
            swalWithBootstrapButtons.fire("Cancelled", "Batal mengirim form permintaan harga :)", "error");
          }
        });
    },
    createDraft() {
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
                    ...
                    .then((res) => {
                      axios
                        ...
                        .then((res) => {
                          ...
                          this.load = false;
                          swalWithBootstrapButtons.fire("Sukses!", "Berhasil menyimpan permintan.", "success");
                          this.$router.push({
                            name: "formprice",
                          });
                        });
                          ...
                          axios
                            ...
                            .then((res) => {
                              axios
                                ...
                                .then((res) => {
                                  this.load = false;
                                  swalWithBootstrapButtons.fire("Sukses!", "Berhasil menyimpan permintaan.", "success");
                                  this.$router.push({
                                    name: "formprice",
                                  });
                                });
            ...
            this.load = false;
            swalWithBootstrapButtons.fire("Cancelled", "Batal menyimpan :)", "error");
          }
        });
    },
    hapus(index) {
    ...
    cekinputrso() {
      axios.get("/api/formprice/" + this.upload.nomor_price + this.upload.kode_groupso).then((res) => {
    ...
    batalkan() {
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios.delete("/api/formprice/" + this.$route.params.nomor).then((res) => {
              axios.get("/api/history/" + this.$route.params.nomor).then((res) => {
    ...
    reqedit() {
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios
              .put("/api/formprice/" + this.$route.params.nomor, {
                status: "Draft",
                tgl_sent: "",
                tgl_kordinator: "",
                kordinator: "",
              ...
              .then((res) => {
                axios
                  ...
                  .then((res) => {
                    this.load = false;
                    swalWithBootstrapButtons.fire("Sukses!", "Silahkan edit permintaan anda di list Draft.", "success");
                    this.$router.push({
                      name: "formprice",
                    });
                  });
          ...
          if (result.isConfirmed) {
            ...
            if (this.lplus === this.lband) {
              axios
                .put("/api/formprice/" + this.$route.params.nomor, {
                  status: "Supervisor",
                  tgl_kordinator: this.DateTime(),
                  kordinator: this.ambiluser.name,
                })
                .then((res) => {
                  for (let i = 0; i < this.listpr.length; i++) {
                  ...
                  axios
                    ...
                    .then((res) => {
                      this.load = false;
                      swalWithBootstrapButtons.fire("Berhasil!", "Permintaan telah di konfirmasi.", "success");
                      this.$router.push({
                        name: "formprice",
                      });
                    });
            ...
            this.load = false;
            swalWithBootstrapButtons.fire("Cancelled", "Batal melakukan konfirmasi permintaan :)", "error");
          }
        });
    },
    spvkonfirmasi() {
          ...
          if (result.isConfirmed) {
            ...
            if (this.lplus === this.lband) {
              axios
                .put("/api/formprice/" + this.$route.params.nomor, {
                  status: "Confirm",
                  spv: this.ambiluser.name,
                  tgl_spv: this.DateTime(),
                })
                .then((res) => {
                  for (let i = 0; i < this.listpr.length; i++) {
                    axios.put("/api/custprice/" + this.listpr[i].id, {
