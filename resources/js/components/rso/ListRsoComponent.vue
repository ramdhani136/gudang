  ...
  <div class="container">
    ...
    <div v-if="akses">
      <div class="row row-cols-2">
        <div class="col-4">
          <div
            v-if="status === 'Draft' || status === 'Confirmed'"
            class="form-group"
          >
      ...
      <button
        v-if="status === 'Draft'"
        @click="showmodal()"
        class="btn btn-orange float-left mt-4 ml-1"
      >
        + Tambah Item
      </button>
          ...
          >
            ...
            <tbody>
              <tr v-for="(lp, index) in listpr" :key="index">
                ...
                <td
                  style="text-align: center"
                  v-if="status === 'So' || status === 'Confirmed'"
                >
                  {{ lp.status }}
                </td>
                <td
      ...
      <div class="row mt-2">
        ...
        <button
          v-if="status === 'Confirmed' && btnedit === true"
          @click="reqedit()"
        ...
        <button
          v-if="load === false && (status === 'Confirmed' || status === 'So')"
          @click="showprint()"
            ...
            <div id="modal-width" class="modal-content">
              ...
              <div class="modal-body">
                ...
                <div class="form-group">
                  <label>Jumlah</label>
                  <input
                    v-model="detail.qty_tdktersedia"
                    type="number"
                    name="qty"
                    autocomplete="off"
                    class="form-control"
              ...
              <div class="modal-body">
                <div class="form-group">
                  ...
                  <select class="form-control" v-model="pilihprint">
                    <option value="Tersedia" :disabled="distersedia">
                      Tersedia
                    </option>
                    <option value="Tidak Tersedia" :disabled="distt">
                      Tidak Tersedia
                    </option>
                  </select>
                </div>
    ...
    <div v-if="akses === false && load === false" class="row akses">
      Anda tidak mempunyai akses untuk membuka halaman ini!
    </div>
  </div>
</template>
  ...
  computed: {
    ...
    Filteredlist() {
      return this.listpr.filter((elem) => {
        return elem.lkode_barang
          .toLowerCase()
          .includes(this.ket.kode.toLowerCase());
      });
    },
  },
  methods: {
        ...
        axios.get("/api/user").then((res) => {
          ...
          axios.get("/api/customer").then((res) => {
            ...
            axios.get("/api/groupso/data/aktif").then((res) => {
              ...
              this.load = false;
              axios
                .get("/api/rso/tersedia/" + this.$route.params.id)
                .then((res) => {
                  ...
                  axios
                    .get("/api/rso/tdktersedia/" + this.$route.params.id)
                    .then((res) => {
                      if (res.data.data.length < 1) {
                        this.distt = true;
                      } else {
          ...
          for (let i = 0; i < this.listpr.length; i++) {
            ...
            if (this.status === "Draft") {
              axios
                .get(
                  "/api/view/price/" +
                    this.listpr[i].kode_customer +
                    "/" +
                    this.listpr[i].lkode_barang
                )
                .then((res) => {
                  ...
                  if (this.$route.params.id !== this.upload.nomor_rso) {
                    ...
                    axios.post("/api/history", {
                      ...
                      notif:
                        ...
                        this.upload.nomor_rs,
                      keterangan:
                        "Mengubah nomor RSO lama : " + this.$route.params.id,
                      jenis: "RSO",
                      tanggal: this.DateTime(),
                      aktif: "N",
                    });
    ...
    print() {
      if (this.pilihprint === "" || this.pilihprint === undefined) {
        ...
        if (this.pilihprint === "Tersedia") {
          var x = window.open(
            "/data/rso/print/" + this.$route.params.id,
            "_blank"
          );
          x.focus();
        } else {
          var x = window.open(
            "/data/rsott/print/" + this.$route.params.id,
            "_blank"
          );
          x.focus();
        }
