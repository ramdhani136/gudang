<template>
  <div class="container" @keyup.esc="defaultfilter">
    ...
    <div class="row">
      <router-link to="/so/new" class="btn btn-success my-3"
        >+ Create SO</router-link
      >
    </div>
        ...
        <tbody>
          <tr v-for="(rs, index) in FilterKategori" :key="index">
            ...
            <td style="text-align: center">
              <router-link
                :to="{ name: 'viewnewso', params: { id: rs.nomor_so } }"
                class="btn btn-none"
              >
                {{ rs.nomor_so }}
              </router-link>
            ...
            <td style="text-align: center">
              <button
                v-if="rs.status !== 'Draft'"
                @click="showhistory(rs)"
              ...
              <button
                ...
                v-if="
                  rs.status !== 'Selesai' &&
                  rs.status !== 'Di Selesaikan' &&
                  ket.aktif[index] == true
                "
              ...
              <button
                @click="requestselesai(rs)"
                v-if="
                  rs.status === 'Acc' &&
                  rs.rs === 'N' &&
                  ket.aktif[index] == false
                "
              ...
              <button
                @click="batalselesai(rs)"
                v-if="rs.status === 'Acc' && rs.rs === 'Y'"
                class="btn btn-none"
              >
                Batal Request
              </button>
    ...
    >
      <div class="modal-dialog" role="document">
        <div id="modal-width" class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">
              Informasi Penolakan SO Selesai
            </h5>
            <button
              @click="resetForm()"
              type="button"
  ...
  computed: {
    FilterKategori() {
      ...
      return _.filter(lists, function (query) {
        ...
        var tanggal =
            vm.filter.listtanggal === "so"
              ? query.tanggal_so >= vm.filter.mulaitanggal &&
                query.tanggal_so <= vm.filter.sampaitanggal
              : query.tanggal_kirim >= vm.filter.mulaitanggal &&
                query.tanggal_kirim <= vm.filter.sampaitanggal,
          ...
          groupin = vm.filter.kode_groupso
            ...
            : true,
          nomorso = vm.filter.nomor
            ? query.nomor_so
                .toLowerCase()
                .includes(vm.filter.nomor.toLowerCase())
            : true,
          nomorrso = vm.filter.nomor
            ? query.nomor_rso
                .toLowerCase()
                .includes(vm.filter.nomor.toLowerCase())
            : true,
  ...
  methods: {
    getSo() {
      if (this.ambiluser.susales === 1) {
        axios.get("/api/so/data/realso").then((res) => {
        ...
        axios
          .get("/api/so/data/view/" + this.ambiluser.kode_groupso)
          .then((res) => {
            this.so = res.data.data;
            this.filter.status = "Draft";
            this.filter.status = "Acc";
    ...
    pilihstatus() {
      for (let i = 0; i < this.FilterKategori.length; i++) {
        axios
          .get("/api/listso/" + this.FilterKategori[i].nomor_so)
          .then((res) => {
    ...
    showhistory(rs) {
      ...
      axios.get("/api/history/data/" + rs.nomor_so + "/So").then((res) => {
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios.delete("/api/so/" + rs.nomor_so).then((res) => {
              axios
                .get("/api/history/data/" + rs.nomor_so + "/So")
                .then((res) => {
    ...
    kirimselesai() {
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.value) {
            if (this.ambiluser.kode_groupso === "GR") {
              this.input.rs = "Y";
              this.input.rss = "Y";
            } else {
              this.input.rs = "Y";
            ...
            axios.put("/api/so/" + this.nso, this.input).then((res) => {
              this.getSo();
              this.input = {};
              axios.post("/api/history", {
                nomor_dok: this.nso,
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.value) {
            ...
            axios.put("/api/so/" + rs.nomor_so, this.input).then((res) => {
              axios.post("/api/history", {
