    ...
    <div class="row row-cols-2">
        <div class="col-4">
            <div class="form-group">
                ...
                <input @input="ceknomor()" v-model="up.nomor_retur" type="text" class="form-control col-12" maxlength="15" :class="{ 'is-valid': bisakirim}" :disabled="statusretur!=='Draft' || ambiluser.sales===0">
            </div>
            <div class="form-group">
                <label>Tanggal :</label>
                <input v-model="up.tanggal" type="date" @change="validate()" :min="now()" class="form-control col-12" :disabled="statusretur!=='Draft' || ambiluser.sales===0">
            </div>
            <div class="form-group">
        ...
        <div class="col-4">
            ...
            <div class="form-group">
                ...
                <textarea v-model="up.keterangan" name="keterangan" class="form-control col-12" :disabled="statusretur!=='Draft' || ambiluser.sales===0"></textarea>
            </div>
        </div>
        <div class="col-4">
            <div class="form-group">
            ...
            <table id="rsthead" class="table mt-2 table-striped table-bordered" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th>Jumlah Retur</th>
                        <th>Keterangan</th>
                        <th v-if="statusretur==='Draft' && ambiluser.sales===1">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(listbcm,index) in checker" :key="index">
                        ...
                        <td style="text-align:center">
                            <input @input="validqty(index)" v-model="hitung.qty[index]" type="number" class="form-control" :disabled="statusretur!=='Draft' || ambiluser.sales===0">
                        </td>
                        <td style="text-align:center">
                            <textarea v-model="hitung.keterangan[index]" class="form-control" :disabled="statusretur!=='Draft' || ambiluser.sales===0"></textarea>
                        </td>
                        <td v-if="statusretur==='Draft' && ambiluser.sales===1" style="text-align:center">
        ...
        getSoAktif() {
            axios.get("/api/retur")
                .then(res => {
                    ...
                    axios.get("/api/listretur/" + this.$route.params.nomor)
                        .then(res => {
                            this.listretur = res.data.data;
                            for (let i = 0; i < this.listretur.length; i++) {
                                axios.get("/api/listso/data/" + this.up.nomor_so + "/" + this.listretur[i].kode_barang)
                                    .then(res => {
                                        ...
                                        axios.get("/api/listretur/bbk/" + this.up.nomor_so + "/" + this.listsoview[i].kode_barang)
                                            .then(res => {
                                                ...
                                                this.terkirim = (parseInt(this.listsoview[i].bbk) - parseInt(this.jumlahlist)) + parseInt(this.listretur[i].qty);
                                                this.checker.push({
