<template>
  <div class="container">
    ...
    <div class="row">
      <router-link to="/bck/create" class="btn btn-success my-3"
        >+ Create Bukti Checker</router-link
      >
    </div>
        ...
        <tbody>
          <tr v-for="(bk, index) in FilteredBck" :key="index">
            ...
            <td style="text-align: center">
              <router-link
                :to="{ name: 'viewbck', params: { bck: bk.bck } }"
                class="btn btn-none"
              >
                {{ bk.bck }}
              </router-link>
            ...
            <td style="text-align: center">
              ...
              <button
                v-if="filter.status === 'open' || filter.status === 'draft'"
                @click="batalkan(bk)"
...
export default {
      ...
      return _.filter(lists, function (query) {
        var tanggal =
          ...
          nomorbck = vm.filter.nomor
            ? query.bck.toLowerCase().includes(vm.filter.nomor.toLowerCase())
            : true,
  ...
  methods: {
    ...
    showhistory(bk) {
      ...
      axios.get("/api/history/data/" + bk.bck + "/Bck").then((res) => {
        ...
        .then((result) => {
          ...
          if (result.isConfirmed) {
            ...
            axios
              ...
              .then((res) => {
                /* edit listso */
                axios.get("/api/listbck/" + bk.bck).then((res) => {
                  ...
                  for (let i = 0; i < this.listbckhps.length; i++) {
                    ...
                    axios
                      ...
                      .then((res) => {
                        ...
                        this.qtybck =
                          parseInt(this.listsohps[0].bck) -
                          parseInt(this.listbckhps[i].qty);
              ...
              .then((res) => {
                axios.delete("/api/bck/" + bk.bck).then((res) => {
                  axios
                    .get("/api/history/" + bk.bck)
                    .then((res) => {
