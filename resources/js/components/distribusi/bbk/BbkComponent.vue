<template>
  <div class="container">
    ...
    <div class="row">
      <router-link to="/distribusi/bbk/create" class="btn btn-success my-3"
        >+ Create BBK</router-link
      >
    </div>
        ...
        <tbody>
          <tr v-for="(bk, index) in FilteredBbk" :key="index">
            ...
            <td style="text-align: center">
              <router-link
                :to="{ name: 'viewbbk', params: { bbk: bk.bbk } }"
                class="btn btn-none"
              >
                {{ bk.bbk }}
              </router-link>
...
export default {
      ...
      return _.filter(lists, function (query) {
        var tanggal =
          ...
          groupin = vm.filter.kode_groupso
            ...
            : true,
          nomorbbk = vm.filter.nomor
            ? query.bbk.toLowerCase().includes(vm.filter.nomor.toLowerCase())
            : true,
  ...
  methods: {
    ...
    showhistory(bk) {
      ...
      axios.get("/api/history/data/" + bk.bbk + "/Bbk").then((res) => {
                                ...
                                .then((res) => {
                                  ...
                                  axios
                                    ...
                                    .then((res) => {
                                      this.listbck = res.data.data;
                                      if (
                                        parseInt(this.listmu[i].bbk) -
                                          parseInt(this.listmu[i].qty) >
                                        0
                                      ) {
                                        this.lebih =
                                          parseInt(this.listmu[i].bbk) -
                                      ...
                                      this.qtybck =
                                        parseInt(this.listbbk[i].qty) -
                                        parseInt(this.lebih);
                                      console.log(this.qtybck);
                                      axios.put(
                                        "/api/listbck/" + this.listbck[0].id,
                                        {
                                          qty: this.qtybck,
                                        }
                                      );
                            ...
                            .then((res) => {
                              ...
                              axios
                                ...
                                .then((res) => {
                                  ...
                                  this.qtybbk =
                                    parseInt(this.listso[0].bbk) -
                                    parseInt(this.listbbk[i].qty);
                                  if (
                                    this.listso[0].bck >= this.listso[0].qty
                                  ) {
                                    this.closeso = "Y";
                                  } else {
                                    this.closeso = "N";
                        ...
                        axios
                          ...
                          .then((res) => {
                            ...
                            for (let i = 0; i < this.listsofix.length; i++) {
                              if (
                                this.listsofix[i].bck >= this.listsofix[i].qty
                              ) {
                                this.a = "Y";
                              } else {
                                this.a = "N";
