...
class GroupItemResource extends JsonResource
{
    ...
    {
        $jumlah= Listrso::where('kode_barang',$this->kode_barang)->where('booking','Y')->where('qty_tdktersedia','>',0)->where('nomor_po',$this->nomor_po)->where('open_po','Y')->sum('qty_tdktersedia');
